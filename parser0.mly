...
%type<Source0.term'> term
%start<Source0.program_with_locations> program
...
%inline definition: 
| LET b=located(binding) EQUAL t=located(term)
{
  (b, t)
}
  ...
  EQUAL t=located(term)
{
  ...
  let ty = match snd (Position.value arg) with
    | None -> None
    | Some ty ->
       Some (TyArrow (ty, oty)) in
  let b = Position.with_pos pos (x, ty) in
  ...
  EQUAL t=located(term)
{
  let pos = Position.position x in
  let x = Position.value x in
  let b = Position.with_pos pos (x, None) in
  (b, Position.(with_pos (position t) (make_lambda_abstraction [arg] t)))
}
...
binding: 
| LPAREN x=identifier COLON ty=typ RPAREN
{
  (x, Some ty)
}
| LPAREN x=identifier RPAREN
{
  (x, None)
}
